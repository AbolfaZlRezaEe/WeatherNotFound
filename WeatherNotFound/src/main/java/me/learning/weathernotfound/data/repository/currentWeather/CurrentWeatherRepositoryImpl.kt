...
internal class CurrentWeatherRepositoryImpl(
    ...
    private lateinit var currentWeatherInformationJob: Job
    ...
    override fun getCurrentWeatherInformation(
        ...
        currentWeatherInformationJob = CoroutineScope(Dispatchers.IO).launch {
    ...
    override fun removeCacheInformationOlderThan(timeStamp: Long) {
        removeCacheInformationJob = CoroutineScope(Dispatchers.IO).launch {
            currentWeatherDao.deleteCurrentWeatherEntitiesOlderThan(timeStamp)
        }
    }

    override fun dispose() {
        if (this::currentWeatherInformationJob.isInitialized
            && !currentWeatherInformationJob.isCompleted
            && !currentWeatherInformationJob.isCancelled
        ) {
            currentWeatherInformationJob.cancel()
        }
    ...
    private suspend fun cacheResponseModelIntoDatabase(
        ...
        if (lastCurrentWeather == null) {
            ...
            currentWeatherDao.updateCurrentWeatherEntity(
                finalEntityModel
            )
        }
    }
